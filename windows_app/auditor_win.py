...
def run_scan(cidr: str, extra: list[str], data_dir: Path) -> int:
    ...
    db_cmd = [
        sys.executable,
        str(Path(__file__).resolve().parent / "scan_library.py"),
        "--db",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Windows launcher for auditor scanner")
    parser.add_argument("--data-dir", default=str(Path.home() / "AuditorData"))
