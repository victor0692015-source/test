...
Или без `.env`, напрямую аргументами:
...
значит бот не нашел `.env` или в нем нет нужных ключей.
...
Запускать можно из любой папки — по умолчанию бот читает `.env` из корня проекта автоматически.

Также можно запустить без `.env`, передав ключи напрямую:

```bash
python bot/main.py --telegram-token "..." --openai-api-key "..."
```

Или указать другой `.env`:

```bash
python bot/main.py --env-file /path/to/.env
```
