...
<title>Auditor Scan Library (macOS)</title>
<style>
...
<div class='top'><div class='logo'>comoda</div><div class='pill'>Scan Library (macOS)</div></div>
<div class='wrap'><h1>macOS Auditor - Scan Library</h1><p>Reports: <code>{html.escape(str(reports_dir))}</code></p><div class='grid'>{''.join(cards)}</div></div>
</body></html>"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="macOS scan library")
    parser.add_argument("--db", default=str(Path.home() / "Library" / "Application Support" / "AuditorData" / "auditor.db"))
    parser.add_argument("--reports", default=str(Path.home() / "Library" / "Application Support" / "AuditorData" / "reports"))
